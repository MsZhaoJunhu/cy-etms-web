            ...
            <a-row :gutter="24">
              ...
              <a-col :span="12">
                <a-form-item :label="`年龄`" class="age">
                  <a-input
                    addonAfter="岁"
                    v-decorator="['sui', { rules: [ { required: false, message: '请输入多少岁', }, ],  }, ]"
                  />
                  <a-input
                    addonAfter="月"
                    v-decorator="['yue', { rules: [ { required: false, message: '请输入多少月', }, ],  }, ]"
                  />
                  <a-input
                    addonAfter="天"
                    v-decorator="['tian', { rules: [ { required: false, message: '请输入多少天', }, ],  }, ]"
                  />
              ...
              <a-col :span="12">
                <a-form-item :label="`医保号`">
                  <a-input
                    v-decorator="[ `medicalInsuranceNo`, { rules: [ { required: false, message: '', }, ],  }, ]"
                    placeholder
                  />
                </a-form-item>
              </a-col>
              ...
              <a-col :span="12">
                <a-form-item :label="`民族`">
                  <a-input
                    v-decorator="[ `nation`, { rules: [ { required: false, message: '', }, ],  }, ]"
                    placeholder
                  />
                </a-form-item>
              </a-col>
              ...
              <a-col :span="12">
                <a-form-item :label="`去向`">
                  <a-radio-group name="whereabouts" :defaultValue="1">
                    <a-input
                      v-decorator="[ `whereabouts`, { rules: [ { required: false, message: '', }, ],  }, ]"
                      placeholder
                    />
                  </a-radio-group>
                </a-form-item>
            ...
            <a-row :gutter="24">
              <a-col :span="8">
                <a-form-item :label="`体温`">
                  <a-input
                    v-decorator="[ `temperature`, { rules: [ { required: true, message: '请输入体温', }, ],  }, ]"
                    placeholder="℃"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item :label="`心率`">
                  <a-input
                    v-decorator="[ `heartRate`, { rules: [ { required: true, message: '请输入心率', }, ],  }, ]"
                    placeholder="次/分"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item :label="`呼吸`">
                  <a-input
                    v-decorator="[ `breathe`, { rules: [ { required: true, message: '请输入呼吸', }, ],  }, ]"
                    placeholder="次/分"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item :label="`血压`">
                  <a-input
                    v-decorator="[ `bloodPressure`, { rules: [ { required: true, message: '请输入血压', }, ],  }, ]"
                    placeholder="mmHg"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item :label="`血糖`">
                  <a-input
                    v-decorator="[ `bloodGlucose`, { rules: [ { required: true, message: '请输入血糖', }, ],  }, ]"
                    placeholder="mmol/L"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="8">
                <a-form-item :label="`脉搏`">
                  <a-input
                    v-decorator="[ `pulse`, { rules: [ { required: true, message: '请输入脉搏', }, ],  }, ]"
                    placeholder="bpm"
                  />
                </a-form-item>
              </a-col>
              <a-col :span="16">
                <a-form-item :label="`血氧饱和度`" class="oxygenSaturation">
                  <a-input
                    v-decorator="[ `oxygenSaturation`, { rules: [ { required: true, message: '血氧饱和度', }, ],  }, ]"
                    placeholder="%"
                  />
                </a-form-item>
              </a-col>
            ...
            <a-row :gutter="24">
              ...
              <a-col :span="24">
                <a-form-item :label="`主要症状`" class="mainSymptoms">
                   <a-textarea v-decorator="[ `mainSymptoms`, { rules: [ { required: true, message: '请输入主要症状', }, ],  }, ]"  placeholder="" :rows="4" />
                </a-form-item>
              </a-col>
              <!-- <a-col :span="12">
              ...
              <a-col :span="24">
                <a-form-item :label="`绿色通道`" class="greenChannel">
                  <a-radio-group
                    v-decorator="['greenChannel',{ rules: [ { required: false, message: '', }, ] },]"
                  >
            ...
            <a-row :gutter="24">
              ...
              <a-col :span="12">
                <a-form-item :label="`初/复诊`">
                  <a-radio-group
                    v-decorator="['firstAndSecondVisit',{ rules: [ { required: true, message: '初/复诊', }, ] },]"
                  >
              ...
              <a-col :span="12">
                <a-form-item :label="`是否手术`">
                  <a-radio-group
                    v-decorator="['isSurgery',{ rules: [ { required: true, message: '是否手术', }, ] },]"
                  >
                    <a-radio value="1">是</a-radio>
                    <a-radio value="0">否</a-radio>
                  </a-radio-group>
...
export default {
  ...
  methods: {
    setFormValues(obj) {
      const that = this;
      // that.triage_patient_info.getFieldDecorator("patientId");
      // that.triage_patient_info.setFieldsValue({patientId:1111});
      ...
      let triagePatientInfo = {};
      let triageInfo = {};
      let triagePatientSymptom = {};
      let triageVitalSigns = {};

      //保存ID
      this.id = obj.triagePatientInfo.id;

      for (var key in obj.triagePatientInfo) {
        this.triage_patient_info.getFieldDecorator(key);
        if (this.cyUtils.inspectionTime(obj.triagePatientInfo[key])) {
          triagePatientInfo[key] = this.$Moment(
            obj.triagePatientInfo[key],
            "YYYY-MM-DD"
          );
          a[key] = this.$Moment(obj.triagePatientInfo[key], "YYYY-MM-DD");
        } else {
          triagePatientInfo[key] = obj.triagePatientInfo[key];
          a[key] = obj.triagePatientInfo[key];
        }
      }
      for (var key in obj.triageInfo) {
        that.triage_info.getFieldDecorator(key);
        if (this.cyUtils.inspectionTime(obj.triageInfo[key])) {
          b[key] = this.$Moment(obj.triageInfo[key], "YYYY-MM-DD");
          triageInfo[key] = this.$Moment(obj.triageInfo[key], "YYYY-MM-DD");
        } else {
          triageInfo[key] = obj.triageInfo[key];
          b[key] = obj.triageInfo[key];
        }
      }
      for (var key in obj.triagePatientSymptom) {
        that.triage_patient_symptom.getFieldDecorator(key);
        if (this.cyUtils.inspectionTime(obj.triagePatientSymptom[key])) {
          c[key] = this.$Moment(obj.triagePatientSymptom[key], "YYYY-MM-DD");
          triagePatientSymptom[key] = this.$Moment(
            obj.triagePatientSymptom[key],
            "YYYY-MM-DD"
          );
        } else {
          c[key] = obj.triagePatientSymptom[key];
          triagePatientSymptom[key] = obj.triagePatientSymptom[key];
        }
      }
      for (var key in obj.triageVitalSigns) {
        that.triage_vital_signs.getFieldDecorator(key);
        if (this.cyUtils.inspectionTime(obj.triageVitalSigns[key])) {
          d[key] = this.$Moment(obj.triageVitalSigns[key], "YYYY-MM-DD");
          triageVitalSigns[key] = this.$Moment(
            obj.triageVitalSigns[key],
            "YYYY-MM-DD"
          );
        } else {
          d[key] = obj.triageVitalSigns[key];
          triageVitalSigns[key] = obj.triageVitalSigns[key];
        }
      }
      this.triage_info.setFieldsValue(triageInfo);
      this.triage_patient_info.setFieldsValue(triagePatientInfo);
      this.triage_patient_symptom.setFieldsValue(triagePatientSymptom);
      this.triage_vital_signs.setFieldsValue(triageVitalSigns);
    },
    //对象转数组
    objArr(obj) {
      let arr = [];
